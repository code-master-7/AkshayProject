...
if (isset($_GET['id']) && isset($_GET['work'])) {
    ...
    switch ($_GET['work']) {
        ...
        case 'delete':
            ...
            $client_get_image_path = "SELECT * FROM `images` WHERE image_id = '$image';";

            $SQL_client_get_image_address = mysqli_query($con_server,$client_get_image_path);

            while($client_Image_Name = mysqli_fetch_assoc($SQL_client_get_image_address)){
                $path1 = "images/".$client_Image_Name['image_url'];
                unlink($path1);
            }
        ...
        case 'deleteproject':
            ...
            $get_image_path = "SELECT * FROM `client` WHERE `project_id` = $id;";

            $SQL_get_image_address = mysqli_query($con_server,$get_image_path);

            while($Image_Name = mysqli_fetch_assoc($SQL_get_image_address)){
                $path = "images/".$Image_Name['logoImage'];
                unlink($path);
            }

            $client_get_image_path = "SELECT * FROM `images` WHERE `project_id` = $id;";
        ...
        case 'deletePost':
            ...
            $client_get_image_path = "SELECT * FROM `images` WHERE post_id = '$_GET[PostId]' AND project_id = '$id';";
            ...
            $query_deleteImages = "DELETE FROM `images` WHERE post_id = '$_GET[PostId]' AND project_id = '$id'";
                ...
                case 'postImage':
                    ...
                    <div class="card position-absolute top-50 start-50 translate-middle" style="width: 18rem;">
                        <div class="card-body">
                            ...
                            <a href="demo.php?id=<?php echo $_GET['id']; ?>" class="btn btn-primary">Cancel</a>
                        </div>
                    </div>
                    <?php
                    break;
